...
Retrieval is FTS5 BM25 with RRF over category and constraint routes. Reranking prefers exact phrases, IDF-weighted term coverage, the leaf category token, and typed color/material matches scaled by how rare that value is in the catalog. A conflicting extracted color or material is a full penalty even for common values. An optional local MiniLM cosine is a light blend (weight 1.0) and is skipped when weights are missing.
