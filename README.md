...
Retrieval is FTS5 BM25 with RRF over category and constraint routes. Reranking prefers exact phrases, IDF-weighted term coverage, and typed color/material matches. Generic catalog lines such as `Imported` or `Button closure` are ignored. An optional local MiniLM cosine is a light blend (weight 1.0) and is skipped when weights are missing.
